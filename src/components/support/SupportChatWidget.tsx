...
function SupportChatWidgetInner({
  ...
  const loadThread = useCallback(async (id: string, token: string | null) => {
  ...
  useEffect(() => {
    if (view === "chat" && chatId) {
      loadThread(chatId, guestToken).then((ok) => {
        if (!ok) {
  ...
  function openWidget() {
    setView((v) => (v === "closed" ? (chatId ? "chat" : "form") : "closed"));
    setError(null);
  }
          ...
          {view === "form" ? (
            <form onSubmit={startChat} className="flex flex-1 flex-col overflow-y-auto p-4">
              ...
              <button
                type="submit"
                disabled={loading}
                className="mt-4 w-full rounded-md bg-gold px-4 py-2.5 text-sm font-semibold text-black hover:bg-gold-light disabled:opacity-60"
              >
              ...
              >
                ...
                <div className="mt-2 flex items-center gap-2">
                  ...
                  <button
                    type="submit"
                    disabled={loading}
                    className="ml-auto rounded-md bg-gold px-3 py-1.5 text-xs font-semibold text-black hover:bg-gold-light disabled:opacity-60"
                  >
                    Send
                  </button>
                </div>
