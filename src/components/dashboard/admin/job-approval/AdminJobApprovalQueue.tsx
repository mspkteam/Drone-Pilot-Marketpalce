...
export function AdminJobApprovalQueue({ initialData }: AdminJobApprovalQueueProps) {
  ...
  const refreshQueue = useCallback(async () => {
    const res = await fetch("/api/admin/jobs?status=pending_approval");
    const data = await res.json();
    if (!res.ok) return;

    const pending = (data.jobs ?? []) as Array<{
      id: string;
      title: string;
  ...
  async function handleConfirm(reason?: string) {
    ...
    setRows((current) => current.filter((row) => row.id !== modal.row!.id));
    ...
    setStats((current) =>
      current.map((card) => {
        ...
        if (card.label === "APPROVED TODAY" && modal.mode === "approve") {
          const next = Number(card.value) + 1;
          return { ...card, value: String(next) };
        }
            ...
            >
              ...
              <div className="admin-job-approval-mission-main">
                <div className="admin-job-approval-mission-top">
                  <span className="admin-job-approval-mission-id">{row.missionId}</span>
                  <span
                  ...
                  {row.isNightOp ? (
                    <span className="admin-job-approval-night-badge">NIGHT OP</span>
                  ) : null}
                </div>
                <h3 className="admin-job-approval-mission-title">{row.title}</h3>
                <div className="admin-job-approval-mission-meta">
                  <span>
                    <span className="admin-job-approval-meta-label">POSTED BY:</span>{" "}
                    {row.postedBy}
                  </span>
                  <span>
                    <span className="admin-job-approval-meta-label">LOCATION:</span>{" "}
                    {row.location}
                  </span>
                  <span>
                    <span className="admin-job-approval-meta-label">BUDGET:</span>{" "}
                    <span className="admin-job-approval-budget">{row.budget}</span>
                  </span>
                </div>
              </div>
